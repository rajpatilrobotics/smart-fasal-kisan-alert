...
import {
  JsonObjectSchema, PurposeCodeSchema, RevisionSchema, RoleTypeSchema,
  Sha256DigestSchema, TimestampSchema, UuidSchema,
} from '../http/common.js';

export const VoiceLanguageSchema = z.enum(['mr', 'hi', 'en']);
export const VoiceSessionStateSchema = z.enum(['CREATED', 'READY', 'RECONNECTING', 'EXPIRING', 'CLOSED', 'UNAVAILABLE']);
...
export const CreateVoiceSessionRequestSchema = z.object({
  ...
  audioCapabilities: z.object({ realtime: z.boolean(), httpsAudio: z.boolean(), offlineAudio: z.boolean() }).strict(),
...
export const CreateVoiceSessionResponseSchema = z.object({
  ...
  websocketEndpoint: z.string().url().refine((value) => value.startsWith('wss://')),
...
export const VoiceTurnResponseSchema = z.object({
  turnId: UuidSchema,
  sessionId: UuidSchema,
  state: z.enum(['HELP', 'UNAVAILABLE', 'NEEDS_CLARIFICATION', 'PROPOSAL_PENDING']),
...
export const VoiceProposalStateSchema = z.enum([
  'PENDING', 'CONFIRMED', 'CANCELLED', 'SUPERSEDED', 'EXPIRED', 'EXECUTING', 'COMPLETE', 'FAILED',
]);
export const VoiceProposalResponseSchema = z.object({
...
export const ConfirmVoiceProposalRequestSchema = VoiceProposalActionBaseSchema.extend({ payloadHash: Sha256DigestSchema })
  .strict().meta({ id: 'ConfirmVoiceProposalRequest', 'x-data-classification': 'C3' });
export const CorrectVoiceProposalRequestSchema = VoiceProposalActionBaseSchema.extend({ correction: JsonObjectSchema })
...
export const CancelVoiceProposalRequestSchema = VoiceProposalActionBaseSchema.strict().meta({
  id: 'CancelVoiceProposalRequest', 'x-data-classification': 'C2',
});
export const VoiceCommandStatusResponseSchema = z.object({
  commandId: UuidSchema,
...
export const VoiceControlFrameSchema = z.object({
  protocolVersion: z.literal(1),
  sessionId: UuidSchema,
  ...
  type: z.enum([
    'session.start', 'audio.end', 'barge_in', 'proposal.confirm', 'proposal.correct',
    'proposal.cancel', 'transport.ack', 'transport.resync_request', 'ping', 'session.close',
    'session.ready', 'state.changed', 'transcript.partial', 'transcript.final', 'clarification',
    'tool.proposal', 'proposal.state', 'command.state', 'validated.result', 'audio.metadata',
    'transport.resync', 'error', 'session.expiring', 'session.closed',
  ]),
