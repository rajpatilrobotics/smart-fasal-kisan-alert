...
function AuthProbe({
  ...
  const { beginSignIn, credentials, providerState, roleContextId, setRoleContextId, signOut } =
    useAuthMemory();
  return (
    <>
      <output>{providerState}</output>
      <output>{credentials?.idToken ?? 'no-credentials'}</output>
      <output>{roleContextId ?? 'no-role-context'}</output>
      <button onClick={() => void beginSignIn(createReturnState)} type="button">
        begin
      </button>
      <button
        onClick={() => setRoleContextId('00000000-0000-4000-8000-000000000201')}
        type="button"
      >
        set role
      </button>
...
describe('Farmer in-memory identity bridge', () => {
  it('reacts to the ready event and accepts only a memory-persistent bridge', async () => {
    const createReturnState = vi.fn().mockResolvedValue('00000000-0000-4000-8000-000000000101');
    ...
    expect(signIn).toHaveBeenCalledWith({
      returnStateId: '00000000-0000-4000-8000-000000000101',
      surface: 'farmer',
    });
    ...
    expect(screen.getByText('no-role-context')).toBeInTheDocument();
  });
});
