...
interface RskWorkShellProps {
  readonly loadState?: typeof loadRskShell;
  readonly revokeRoleContext?: typeof revokeRskRoleContext;
}
type RenderState = RskShellState | { readonly kind: 'loading' };
...
export function RskWorkShell({
  loadState = loadRskShell,
  revokeRoleContext = revokeRskRoleContext,
}: RskWorkShellProps) {
  ...
  return (
    <div className="rsk-app" lang={locale}>
      <a className="skip-link" href="#main-content">
        {copy.skipToContent}
      </a>
      <aside className="authenticated-rail" aria-label="RSK">
        <strong>{copy.appName}</strong>
        <span>RSK Operations</span>
        <p className="rail-boundary-copy">{copy.rskProtectedBody}</p>
      </aside>
      <div className="authenticated-canvas">
        <header className="simple-header">
          <span>Milestone 1 · authenticated shell</span>
          {credentials ? (
            <button
        ...
        <main id="main-content" className="authenticated-workspace">
          <p className="eyebrow">RSK · Secure work context</p>
          <h1 ref={headingRef} tabIndex={-1}>
            {copy.rskWorkHeading}
          </h1>
          {viewState.kind === 'ready' ? (
            <div className="ready-layout">
              <p className="lead">{copy.rskWorkBody}</p>
              <dl className="context-grid">
                ...
                <div>
                  <dt>{copy.contextOffice}</dt>
                  <dd>{viewState.officeId}</dd>
                </div>
                <div>
              ...
              <section className="boundary-card" aria-labelledby="protected-boundary-title">
                <h2 id="protected-boundary-title">{copy.rskProtectedTitle}</h2>
                <p>{copy.rskProtectedBody}</p>
                <p className="technical-state">{viewState.workState}</p>
              </section>
            </div>
