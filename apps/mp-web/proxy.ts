...
export function proxy(request: NextRequest) {
  const firebasePolicy = firebaseBrowserPolicyOrigins(
    process.env.NEXT_PUBLIC_MP_FIREBASE_AUTH_DOMAIN,
  );
  const { contentSecurityPolicy, nonce } = createNonceContentSecurityPolicy({
    connectOrigins: [
      process.env.NEXT_PUBLIC_DOMAIN_API_ORIGIN,
      process.env.NEXT_PUBLIC_MP_QUERY_API_ORIGIN,
      ...firebasePolicy.connectOrigins,
    ],
